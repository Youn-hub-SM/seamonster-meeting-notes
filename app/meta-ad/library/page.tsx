            ...
            <div className="sm-row" style={{ gap: 6 }}>
              {(["영상", "이미지"] as Format[]).map((t) => (
                <button key={t} type="button" onClick={() => set("format", t)}
                  style={{ fontSize: 15, padding: "7px 14px", borderRadius: 8, cursor: "pointer", fontWeight: f.format === t ? 700 : 500, border: f.format === t ? "1px solid var(--sm-orange)" : "1px solid var(--sm-border)", background: f.format === t ? "var(--sm-orange-light)" : "var(--sm-white)", color: f.format === t ? "var(--sm-orange-hover)" : "var(--sm-text-mid)" }}>{t}</button>
              ))}
            </div>
          </label>
          ...
          {list.map((c) => (
            <div key={c.id} className="mlib-card">
              <div className="sm-row" style={{ justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
                <div style={{ fontWeight: 700, fontSize: 15, color: "var(--sm-dark)" }}>{c.name}</div>
                <span style={{ fontSize: 15, fontWeight: 700, padding: "2px 7px", borderRadius: 6, background: c.format === "영상" ? "var(--sm-info-bg)" : "var(--sm-success-bg)", color: c.format === "영상" ? "var(--sm-info)" : "var(--sm-success)", whiteSpace: "nowrap" }}>{c.format}</span>
              </div>
              {(c.roas || c.adLibraryUrl) && (
                <div className="sm-row" style={{ gap: 8, marginTop: 5, fontSize: 15, flexWrap: "wrap", alignItems: "center" }}>
                  {c.roas ? <span className="sm-faint">ROAS <b style={{ color: "var(--sm-orange)" }}>{c.roas.toFixed(2)}</b></span> : null}
                  {c.adLibraryUrl ? <a href={c.adLibraryUrl} target="_blank" rel="noreferrer" style={{ color: "var(--sm-info)", fontSize: 15 }}>소재 보기</a> : null}
                </div>
              )}
              ...
              {c.note && <div className="sm-faint" style={{ fontSize: 15, marginTop: 6 }}>{c.note}</div>}
              <div className="sm-row" style={{ gap: 8, marginTop: 10, justifyContent: "flex-end" }}>
                <button className="b2b-link-btn" style={{ fontSize: 15 }} onClick={() => reuse(c)}>재사용(복제)</button>
                <button className="b2b-link-btn" style={{ fontSize: 15, color: "var(--sm-orange)" }} onClick={() => del(c.id)}>삭제</button>
              </div>
            </div>
          ))}
        </div>
