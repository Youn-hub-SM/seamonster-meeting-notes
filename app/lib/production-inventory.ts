...
// 권장 생산량 = max(0, 수요 + 안전재고 − (현재고 + 오는 중)) — '오는 중'은 열린 제조사 요청서의 잔여
...
export interface InvRow {
  ...
  inbound: number;        // 오는 중 = 열린 제조사 요청서 잔여(소매·전체 수식만, 도매 수식은 0)
...
export interface InventoryResult {
  ...
  inboundOk: boolean;        // 오는 중 집계 성공 여부 — false 면 권장이 실제보다 클 수 있다(화면이 경고)
...
export async function getInventoryRows(channel?: "소매" | "도매"): Promise<InventoryResult> {
  ...
  // 오는 중을 SKU 로 합산(중복 SKU 는 수요와 같은 규칙으로 합쳐진다)
