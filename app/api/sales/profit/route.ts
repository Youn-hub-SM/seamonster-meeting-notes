...
export async function GET(req: NextRequest) {
  try {
    ...
    const [{ data: sumRows, error: e1 }, { data: unm, error: e2 }] = await Promise.all([
      sb.rpc("sales_profit_summary", { p_from: from, p_to: to }),
      sb.rpc("sales_profit_unmatched", { p_from: from, p_to: to }),
    ]);
    if (e1) return NextResponse.json({ ok: false, error: `집계 오류: ${e1.message}. 044 적용 여부 확인.` }, { status: 500 });
    ...
    const rows = ((sumRows as ProfitInput[]) || []).map((r) => computeProfitRow({
      ...
      pay_amount: Number(r.pay_amount) || 0, ship_revenue: Number(r.ship_revenue) || 0,
      product_cost: Number(r.product_cost) || 0, cooling: Number(r.cooling) || 0, fee_rate: Number(r.fee_rate) || 0,
    }));
