...
// PUT { sku, delta, memo?, until? } — SKU별 안전재고 수동 보정 저장.
//  delta 0 이고 메모 없으면 보정 제거(기본 복귀).
export async function PUT(req: NextRequest) {
  try {
    const body = (await req.json()) as { sku?: string; delta?: number; memo?: string; until?: string | null };
