...
//  생산관리자가 "무엇이 생산 전/중/완료인지" 체크하는 보드.
...
export async function GET() {
  try {
    ...
    const sel = "id, order_no, production_status, production_date, companies:company_id(name), order_items(product_name, spec, qty, sort_order)";
    ...
    const orderCards = ([...(pendingRes.data ?? []), ...(doneRes.data ?? [])] as unknown as OrderRow[]).map((o) => {
      ...
      const items = [...(o.order_items ?? [])]
        ...
        .map((it) => ({ name: it.product_name, spec: it.spec || "", qty: Number(it.qty) || 0 }));
      return {
        id: o.id,
        kind: "order" as const,
    ...
    const manualCards = manual.map((m) => ({
      ...
      items: [{ name: m.name, spec: "", qty: m.qty }],
    }));
