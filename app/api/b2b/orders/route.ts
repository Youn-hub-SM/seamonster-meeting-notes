...
export async function POST(req: NextRequest) {
  try {
    ...
    let totalBoxes = 0;
    try {
      const res = await saveOrderShipments(orderRow.id, body.recipient, body.shipments, savedItems, Math.max(1, Math.floor(Number(body.box_count) || 1)), body.ship_date, body.status);
