...
export default function InventoryMovePage() {
  ...
  // 소매→도매 배정 패널 — 열린 도매 생산 요청서 + 요청서별 입력값(item_id → 문자열)
  const [targets, setTargets] = useState<Target[]>([]);
  const [alloc, setAlloc] = useState<Map<string, string>>(new Map());
  ...
  const loadTargets = useCallback(async (productId: string) => {
    try {
      const j = await (await fetch(`/api/inventory/move/targets?product_id=${encodeURIComponent(productId)}`, { cache: "no-store" })).json();
      setTargets(j.ok ? (j.targets || []) : []);
    } catch { setTargets([]); }
  }, []);
  // 품목·방향이 바뀌면 배정 입력 초기화 + (소매→도매일 때만) 요청서 목록 로드
  useEffect(() => {
    setAlloc(new Map());
    if (!pid || dir.from !== "소매") { setTargets([]); return; }
    loadTargets(pid);
  }, [pid, dir.from, loadTargets]);
  ...
  // 배정 합계(현재 목록의 입력값만) — 이동 수량 초과면 저장 불가, 미달분은 자동 기타
  const allocSum = useMemo(() => {
    let s = 0;
    for (const t of targets) s += Math.max(0, Math.round((Number(alloc.get(t.item_id)) || 0) * 100) / 100);
  ...
  async function submit() {
    ...
    if (allocOver) { setError(`배정 합계(${allocSum.toLocaleString()})가 이동 수량(${nQty.toLocaleString()})보다 많습니다. 배정을 줄이세요.`); return; }
    const allocations = targets
      .map((t) => ({ item_id: t.item_id, qty: Math.max(0, Math.round((Number(alloc.get(t.item_id)) || 0) * 100) / 100) }))
      .filter((a) => a.qty > 0);
    setBusy(true);
    try {
      const res = await fetch("/api/inventory/move", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ product_id: pid, from: dir.from, to: dir.to, qty: nQty, txn_date: date, memo, allocations }),
      });
      const j = await res.json();
      if (!res.ok || !j.ok) throw new Error(j.error || "이동 실패");
      const etc = Math.round((nQty - allocSum) * 100) / 100;
      setOk(`${plabel || "품목"} ${nQty}개를 ${dir.from} → ${dir.to} 로 옮겼어요.${allocations.length ? ` (요청서 ${allocations.length}건에 ${allocSum.toLocaleString()} 배정${etc > 0 ? ` · 기타 ${etc.toLocaleString()}` : ""})` : ""}`);
      if (Array.isArray(j.warnings) && j.warnings.length) setError(j.warnings.join(" · "));
      setQty(""); setMemo(""); setAlloc(new Map());
      await loadStock();
      if (dir.from === "소매") await loadTargets(pid);
  ...
  async function cancelMove(group_id: string) {
    ...
    await loadStock();
    if (pid && dir.from === "소매") await loadTargets(pid); // 취소로 요청서 잔여·상태가 바뀌었을 수 있음
  }
      ...
      <section className="b2b-card" style={{ marginBottom: 16 }}>
        ...
        <div className="b2b-field-row" style={{ marginTop: 12 }}>
          <div className="b2b-field">
            ...
            <input className="b2b-input b2b-money" type="number" min={0.01} step={0.01} value={qty} onChange={(e) => setQty(e.target.value)} placeholder="0" />
        ...
        {dir.from === "소매" && pid && (
          <div className="b2b-field" style={{ marginTop: 12 }}>
            <label className="b2b-field-label">도매 생산 요청서 배정 <span className="sm-faint" style={{ fontWeight: 400 }}>(선택 — 비우면 전량 기타)</span></label>
            {targets.length === 0 ? (
              <p className="sm-faint" style={{ fontSize: 12, margin: "4px 0 0" }}>이 품목의 열린 도매 생산 요청이 없습니다 — 전량 기타(요청 미연결)로 기록됩니다.</p>
            ) : (
              <>
                          ...
                          <tr key={t.item_id}>
                            ...
                            <td className="num">
                              <input className="b2b-input b2b-money" type="number" min={0} step={0.01} value={v}
                                onChange={(e) => setAlloc((prev) => { const m2 = new Map(prev); m2.set(t.item_id, e.target.value); return m2; })}
                ...
                <p className="sm-faint" style={{ fontSize: 12, marginTop: 2 }}>배정으로 요청서가 100% 채워지면 자동으로 완료되어 도매 요청 종합에서 빠집니다. 이동을 취소하면 배정도 함께 돌아옵니다.</p>
              </>
            )}
          </div>
        )}
