...
export default function SalesReportPanel({ autoGenerate = false }: { autoGenerate?: boolean }) {
  ...
  // 최신일 확보되면 1회 자동 생성(업로드 직후 인라인용)
  useEffect(() => {
    if (autoGenerate && ready && base && !rpt && busy === "") generate();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [autoGenerate, ready, base]);

  async function generate(m: "daily" | "weekly" = mode, b: string = base) {
    setBusy("gen"); setErr(""); setRpt(null); setSent(null);
    try {
      const q = b ? `?base=${b}` : "";
      const r = await fetch(`/api/sales/report/${m}${q}`);
  ...
  function switchMode(m: "daily" | "weekly") { setMode(m); setRpt(null); setSent(null); if (base) generate(m, base); }
      ...
      {rpt?.ok && (
        <section className="b2b-card" style={{ marginTop: 12 }}>
          ...
          <div style={{ marginTop: 14, borderTop: "1px solid var(--sm-border)", paddingTop: 14 }}>
            <label className="sm-faint" style={{ fontSize: 15, display: "block", marginBottom: 6 }}>수신자 (비우면 기본 수신자에게 발송, 쉼표로 여러 명)</label>
