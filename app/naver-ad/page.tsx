      ...
      <header className="b2b-page-head">
        ...
        {changes.length > 0 && (
          <div className="b2b-page-actions sm-row" style={{ gap: 8, alignItems: "center" }}>
            {savedMsg && <span style={{ fontSize: 15, color: "var(--sm-success)" }}>{savedMsg}</span>}
      ...
      {status?.connected && (
        <>
          ...
          <div style={{ border: "1px solid var(--sm-border)", borderRadius: 12, padding: "12px 14px", marginBottom: 10 }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: "var(--sm-dark)", marginBottom: 8 }}>광고유형</div>
            <div className="sm-row" style={{ gap: 6, flexWrap: "wrap" }}>
              {presentTypes.map(({ t, n }) => (
                <Chip key={t} on={adType === t} onClick={() => changeType(t)}>
                  {TYPE_LABEL[t] || t} <span style={{ fontSize: 15, opacity: 0.6 }}>{n}</span>
                </Chip>
              ))}
            </div>
            ...
            <div className="sm-row" style={{ justifyContent: "space-between", marginBottom: 8 }}>
              ...
              {campsOfType.length > 0 && (
                <div className="sm-row" style={{ gap: 10 }}>
                  ...
                  {selCamp.length > 0 && <button className="b2b-link-btn" style={{ fontSize: 15 }} onClick={() => setSelCamp([])}>선택 해제</button>}
                </div>
              )}
            </div>
            {campsOfType.length === 0 ? <span className="sm-faint" style={{ fontSize: 15 }}>이 유형의 캠페인이 없습니다.</span> : (
              <div className="sm-row" style={{ gap: 6, flexWrap: "wrap" }}>
          ...
          {mode === "keyword" && selCamp.length > 0 && (
            <div style={{ border: "1px solid var(--sm-border)", borderRadius: 12, padding: "12px 14px", marginBottom: 10 }}>
              <div className="sm-row" style={{ justifyContent: "space-between", marginBottom: 8 }}>
                <span style={{ fontSize: 15, fontWeight: 700, color: "var(--sm-dark)" }}>광고그룹 {loadingGrp ? "…" : `(${adgroups.length})`}</span>
                <div className="sm-row" style={{ gap: 10 }}>
                  <button className="b2b-link-btn" style={{ fontSize: 15 }} onClick={() => setSelGrp(adgroups.map((g) => g.nccAdgroupId))}>전체 선택</button>
                  {selGrp.length > 0 && <button className="b2b-link-btn" style={{ fontSize: 15 }} onClick={() => setSelGrp([])}>해제</button>}
                </div>
              </div>
              {adgroups.length === 0 ? <span className="sm-faint" style={{ fontSize: 15 }}>{loadingGrp ? "불러오는 중..." : "그룹이 없습니다."}</span> : (
                <div className="sm-row" style={{ gap: 6, flexWrap: "wrap" }}>
          ...
          <div className="sm-row" style={{ gap: 6, flexWrap: "wrap", alignItems: "center", marginBottom: 8 }}>
            <span style={{ fontSize: 15, fontWeight: 700, color: "var(--sm-dark)", marginRight: 2 }}>기간</span>
            ...
            {!customMode && <span className="sm-faint" style={{ fontSize: 15, whiteSpace: "nowrap" }}>{rangeLabel(preset)}</span>}
            {customMode && (
              <>
          ...
          {/* 전환 기준 (전체 vs 구매) */}
          <div className="sm-row" style={{ gap: 6, flexWrap: "wrap", alignItems: "center", marginBottom: 8 }}>
            <span style={{ fontSize: 15, fontWeight: 700, color: "var(--sm-dark)", marginRight: 2 }}>전환 기준</span>
                  ...
                  <table className="b2b-table">
                      ...
                      {shownRows.map((r) => {
                        ...
                        return (
                          <tr key={r.id} style={changed ? { background: "var(--sm-orange-light)" } : undefined}>
                            <td>
                              <strong>{r.name}</strong>{r.userLock ? <span className="sm-faint" style={{ fontSize: 15 }}> · OFF</span> : null}
                              <button className="b2b-link-btn" style={{ marginLeft: 6, fontSize: 15 }} onClick={() => { setRpt({ id: r.id, name: r.name, kind: r.kind }); setRptGran("week"); setRptBack(90); setRptConv("all"); }} title="이 대상의 주/월/일별 성과 리포트">리포트</button>
                              {r.kind === "adgroup" && <button className="b2b-link-btn" style={{ marginLeft: 6, fontSize: 15 }} onClick={() => openSearchKeywords(r)} title="이 그룹으로 유입된 검색어별 비용">검색어</button>}
                            </td>
                            <td style={{ fontSize: 15, color: "var(--sm-text-light)", maxWidth: 130, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{r.group}</td>
                            ...
                            {mode === "keyword" && <td className="num" style={{ fontSize: 15, color: "var(--sm-text-mid)" }}>{r.adRel ?? "-"}/{r.expClick ?? "-"}</td>}
                            <td style={{ whiteSpace: "nowrap" }}>
                              <div className="sm-row" style={{ gap: 4, alignItems: "center" }}>
                                <button className="b2b-btn-secondary" style={{ padding: "2px 6px", fontSize: 15 }} onClick={() => bump(r, -10)} disabled={useGroup} title="-10%">−</button>
                                ...
                                <button className="b2b-btn-secondary" style={{ padding: "2px 6px", fontSize: 15 }} onClick={() => bump(r, 10)} disabled={useGroup} title="+10%">+</button>
                                {r.kind === "keyword" && (
                                  <label className="sm-row" style={{ gap: 3, fontSize: 15, cursor: "pointer" }} title="그룹 기본입찰가 사용">
                    ...
                    <tfoot>
                      <tr style={{ borderTop: "2px solid var(--sm-border)", fontWeight: 700, background: "var(--sm-bg-subtle)" }}>
                        <td>합계</td><td style={{ fontSize: 15, color: "var(--sm-text-light)" }}>{shownRows.length}개</td>
          ...
          <div onClick={(e) => e.stopPropagation()} style={{ background: "var(--sm-white)", borderRadius: 12, width: "min(880px, 100%)", maxHeight: "90vh", display: "flex", flexDirection: "column", boxShadow: "0 10px 40px rgba(0,0,0,0.2)" }}>
            ...
            <div style={{ padding: "12px 16px", overflow: "auto" }}>
              <div className="sm-row" style={{ gap: 6, flexWrap: "wrap", alignItems: "center", marginBottom: 12 }}>
                <span style={{ fontSize: 15, fontWeight: 700, color: "var(--sm-dark)", marginRight: 2 }}>단위</span>
                {(([["day", "일별"], ["week", "주별"], ["month", "월별"]]) as [("day" | "week" | "month"), string][]).map(([g, l]) => <Chip key={g} on={rptGran === g} onClick={() => setRptGran(g)}>{l}</Chip>)}
                <span style={{ width: 10 }} />
                <span style={{ fontSize: 15, fontWeight: 700, color: "var(--sm-dark)", marginRight: 2 }}>조회</span>
