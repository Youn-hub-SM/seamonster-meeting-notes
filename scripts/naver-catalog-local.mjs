// 네이버 등록 카탈로그 — 로컬 동기화 스크립트 (클로드 코드/터미널에서 실행)
//
// 커머스API 는 애플리케이션에 등록된 IP 에서만 호출이 허용된다(GW.IP_NOT_ALLOWED).
// Vercel 은 고정 IP 가 없어 서버 동기화가 막히므로, 등록 IP 인 이 PC 에서 네이버를 호출해
// 서버 업로드 라우트(/api/naver/catalog/upload)로 밀어넣는다.
//
// 실행: 저장소 루트에서  node scripts/naver-catalog-local.mjs  [서버URL]
//  - 자격증명은 .env.local 의 NAVER_COMMERCE_CLIENT_ID / NAVER_COMMERCE_CLIENT_SECRET 을 읽는다.
...
if (!id || !secret) {
  console.error("[중단] .env.local 에 NAVER_COMMERCE_CLIENT_ID / NAVER_COMMERCE_CLIENT_SECRET 을 추가하세요.");
  process.exit(1);
}
