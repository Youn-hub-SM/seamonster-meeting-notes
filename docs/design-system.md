...
### 카피 규칙 — 부제(`b2b-page-subtitle`)는 설명이 아니다

**"이 화면이 무엇을 하는지" 설명·사용법 안내·`(전체 N건)` 카운트·사이드바와 중복인 링크를 부제에 쓰지 않는다.**
사용자는 이미 알고, 텍스트가 많으면 화면이 복잡해진다(2026-07 사용자 결정으로 68곳 전수 삭제).
부제가 허용되는 경우는 둘뿐 — 어느 쪽이든 **한 줄 이하**:
1. **집계·계산 기준** — 지우면 숫자를 오독하는 것. 예: "발주일 기준 · 취소 제외", "각 칸 = 그 시점까지의 누적 생산량"
2. **데이터 부작용 규칙** — 모르면 실수하는 것. 예: "단가는 적용 시작일별로 관리 — 소급 적용되지 않습니다"

부제 자리에 설명이 아닌 **데이터**(거래처 상세의 사업자번호 등)나 **조건부 상태 경고**(발주 복제 직후)는 예외.
