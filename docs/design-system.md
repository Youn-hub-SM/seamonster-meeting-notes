...
12. **한 클래스를 `<button>` 과 `<a>` 가 같이 쓰면 `line-height` 를 반드시 명시** — 폼 컨트롤은 부모의 글꼴을
    물려받지 않아, `font-size` 만 맞추고 `line-height` 를 빼면 버튼 쪽만 줄 높이가 달라져 같은 목록인데 행 높이가
    어긋난다. 좌측 메뉴(`.app-sb-tool`)가 실제로 이 상태였다 — 하위메뉴 있는 항목은 `<button>`, 없는 항목은 `<a>`.
...
- 타이포: `--sm-fs-xs~2xl` = 12 / 15 / 15(본문) / 15 / 17 / 19 / 23
  - **본문 최소 15px** — 화면의 읽는 글은 15 미만 금지 (2026-08-07 대표 지시로 전면 상향).
  - **보조 정보는 12px** — 발주번호·SKU·날짜 힌트·배지(pill)·차트 축 라벨처럼 곁들이는 글. 12 미만은 어디에도 금지.
  - 예외: 인쇄물(거래명세표 statement, 인쇄 팝업 fulfill/scan, `@media print` 블록)과 파도소리(factory)는 종이 밀도 기준이라 기존 크기 유지.
...
## 10. 2026-08-28 확정 결정(대표)

- **숫자·금액 열은 우측 정렬** — `.b2b-table .num` 이 right (구 표준 left 에서 변경). 화면에서 인라인 textAlign 을 새로 얹지 말 것.
- **'취소' 상태 색 = 중립 회색**(bg-subtle/text-mid) — 오류가 아닌 종결 상태. 발주·생산요청 색 지도 통일.
- **표 본문 폰트 기본 15px** — 예외(밀도 필요)로 채널별 이익·배송일지·발송 통계 표만 12px 유지.
- **표 날짜는 `format.ts dateShort()`("MM-DD")** — 여러 해가 섞이는 이력 화면만 연도 포함 예외.
- **모달 Esc 닫기는 `use-esc.ts useEscClose(onClose)`** — 컴포넌트형 모달에 1줄. 화면별 자작 금지.
- **진행 문구 말줄임은 "..."(점 3개)** — "저장 중...", "불러오는 중...". "…" 문자 사용 금지.
